...
import { names } from "../../data/GetName";
...
const SayHello = () => {
  ...
  useEffect(() => {
    const typed = new Typed(guestRef.current, {
      strings: names.map((n) => {
        if (n == "?") {
          return "Siapakah Kamu ?";
        }
        if (n == "=") {
          return "Biar ku tebak ?!";
        }
        return `Apakah kamu ${n}?`;
      }),
      loop: false,
      backDelay: 1000,
      typeSpeed: 70,
    ...
    <div className="w-full sm:min-w-xl sm:max-w-xl md:mt-8 h-auto px-4 py-2 flex flex-col items-center rounded-md bg-clip-padding backdrop-filter backdrop-blur-sm bg-opacity-10 border border-[#00eaff] shadow-[0_0_12px_#00eaff] z-50">
      ...
      <button
        ...
        onClick={() => {
          ...
          if (BadWords.includes(name.trim().toLowerCase())) {
            Alert("Nama mengandung kata-kata kasar.");
            return;
          }
